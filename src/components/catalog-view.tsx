          ...
          <div className="grid lg:grid-cols-[1fr_280px]">
            ...
            {data.category?.imageUrl ? (
              <div className="relative hidden min-h-[220px] bg-petrol-soft/40 lg:block">
                <ProductImage
                  src={normalizeImageSrc(data.category.imageUrl)}
          ...
          <section className="mb-10">
            <div className="mb-5 flex items-end justify-between gap-4">
              <h2 className="text-2xl font-black text-graphite">
                {data.category ? "Подкатегории" : "Популярные категории"}
              </h2>
              {!data.category ? (
        ...
        <div className="grid gap-8 lg:grid-cols-[320px_1fr]">
          ...
          <section>
            <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
              ...
              {data.pages > 1 ? (
                <div className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-petrol">
            ...
            {data.products.length === 0 ? (
              <div className="rounded-[30px] border border-border bg-white p-10 text-center text-muted">
                Товары не найдены. Попробуйте изменить фильтры или выберите другую категорию.
              </div>
            ) : null}
            {data.pages > 1 ? (
                ...
                {Array.from({ length: data.pages }).map((_, index) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`h-10 w-10 rounded-full text-center text-sm font-bold leading-10 ${page === data.page ? "bg-petrol text-white" : "bg-white text-petrol"}`}
                    >
                      {page}
                    </Link>
                  );
